...
{
    public class StartsWithAccessSpecifierSpecification : Specification<string>
    {
        public override bool IsSatisfiedBy(string codeLine)
        {
            string codeLineTrimmed = Utilities.GetSpacesRemoved(codeLine);
            return Constants.AccessSpecifierCharSet.Any(codeLineTrimmed.ToLower().StartsWith);
        }
    }
}
